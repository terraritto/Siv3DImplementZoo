...
WeightedAStarComponent::WeightedAStarComponent(std::weak_ptr<Actor> owner, int drawOrder)
	: GraphBaseComponent(owner, drawOrder)
{
}

void WeightedAStarComponent::Construct(Array<Array<std::shared_ptr<SearchTile>>> tiles)
{
	// graphをサイズ分初期化
...
void WeightedAStarComponent::Calculate()
{
	...
	std::shared_ptr<AStarScratch> scratch = std::make_shared<AStarScratch>();
	...
	do
	{
		...
		for (const std::shared_ptr<WeightedEdge> edge : current->m_edges)
		{
			...
			if (data == nullptr)
			{
				m_nodeMap[pEdgeTo] = std::make_shared<AStarScratch>();
			...
			if (data->m_isClosedSet == false)
			{
				...
				if (data->m_isOpenSet == false)
				{
					...
					// heuristicを計算
					data->m_heuristic = ComputeHeuristic(edge->m_to) * m_weight; // h * weightにするだけ
		...
		auto iter = std::min_element(openSet.begin(), openSet.end(),
			...
			{
				float fOfA = m_nodeMap[a]->m_heuristic + m_nodeMap[a]->m_actualFromStart;
				float fOfB = m_nodeMap[b]->m_heuristic + m_nodeMap[b]->m_actualFromStart;
				return fOfA < fOfB;
			}
		);
...
double WeightedAStarComponent::ComputeHeuristic(std::weak_ptr<WeightedGraphNode> target) const
{
