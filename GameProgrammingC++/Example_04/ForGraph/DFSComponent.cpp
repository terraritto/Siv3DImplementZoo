...
DFSComponent::DFSComponent(std::weak_ptr<Actor> owner, int drawOrder)
	: GraphBaseComponent(owner, drawOrder)
{
}

void DFSComponent::Construct(Array<Array<std::shared_ptr<SearchTile>>> tiles)
{
	// graphをサイズ分初期化
...
void DFSComponent::Calculate()
{
	m_nodeMap.clear();

	// 辿っていくqueue
	std::stack<std::shared_ptr<GraphNode>> s;
	s.emplace(m_start);
