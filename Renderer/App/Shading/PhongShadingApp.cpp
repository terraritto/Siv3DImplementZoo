...
namespace TerakoyaRenderer::Shading
{
	void PhongShadingApp::Prepare()
	{
		...
		m_vertexShader = std::make_unique<PhongVertexShader>();
		m_pixelShader = std::make_unique<PhongPixelShader>();
		m_pixelShader->SetImage(&m_renderImage);

		if (auto shaderInput = std::static_pointer_cast<PhongVertexInput>(m_vertexShader->GetShaderInput().lock()))
		{
	...
	void PhongShadingApp::Render()
	{
		for (auto& model : m_modelList)
		{
			...
			while (nodes.empty() == false)
			{
				...
				Array<std::shared_ptr<PhongVertexOutput>> tempVertexOutputs;
				...
				for (auto& primitive : primitives)
				{
					...
					{
						auto shaderInput = std::static_pointer_cast<PhongVertexInput>(m_vertexShader->GetShaderInput().lock());
						...
						auto shaderOutput = std::static_pointer_cast<PhongVertexOutput>(m_vertexShader->GetShaderOutput().lock());
						...
						tempVertexOutputs[i] = std::make_shared<PhongVertexOutput>(*shaderOutput);
						i++;
					}
	...
	void PhongShadingApp::FillDrawProcess(const Array<std::shared_ptr<PhongVertexOutput>>& tempVertices, const Array<unsigned int>& tempIndices)
	{
		auto shaderInput = std::static_pointer_cast<PhongPixelInput>(m_pixelShader->GetShaderInput().lock());
	...
	double PhongShadingApp::CalculateEdge(const Vec2& p, const Vec2& origin, const float dx, const float dy) const
	{
		return (p.x - origin.x) * dy - (p.y - origin.y) * dx;
	}

	bool PhongShadingApp::IsInside(const Vec2& p, const Vec2& origin, const double dx, const double dy) const
	{
