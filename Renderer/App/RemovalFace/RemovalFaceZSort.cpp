...
{
	void RemovalFaceZSortApp::Prepare()
	{
		...
		m_vertexShader = std::make_unique<RemovalFaceVertexShader>();
		m_pixelShader = std::make_unique<RemovalFacePixelShader>();

		if (auto shaderInput = std::static_pointer_cast<RemovalFaceVertexInput>(m_vertexShader->GetShaderInput().lock()))
		{
	...
	void RemovalFaceZSortApp::Render()
	{
		for (auto& model : m_modelList)
		{
			...
			while (nodes.empty() == false)
			{
				...
				Array<std::shared_ptr<RemovalFaceVertexOutput>> tempVertexOutputs;
				...
				for (auto& primitive : primitives)
				{
					...
					{
						auto shaderInput = std::static_pointer_cast<RemovalFaceVertexInput>(m_vertexShader->GetShaderInput().lock());
						...
						auto shaderOutput = std::static_pointer_cast<RemovalFaceVertexOutput>(m_vertexShader->GetShaderOutput().lock());
						...
						tempVertexOutputs[i] = std::make_shared<RemovalFaceVertexOutput>(*shaderOutput);
						i++;
					}
	...
	void RemovalFaceZSortApp::FillDrawProcess(const Array<std::shared_ptr<RemovalFaceVertexOutput>>& tempVertices, const Array<unsigned int>& tempIndices)
	{
		Array<std::tuple<double, int>> zSortList;
		// ZSort
		...
		auto shaderInput = std::static_pointer_cast<RemovalFacePixelInput>(m_pixelShader->GetShaderInput().lock());
		...
		{
			auto [depth, index] = sortData;
			
			const auto& vert1 = tempVertices[tempIndices[index]];
			const auto& vert2 = tempVertices[tempIndices[index + 1]];
			const auto& vert3 = tempVertices[tempIndices[index + 2]];

			Vec3 normal = Cross((vert2->worldPos - vert1->worldPos).xyz(), (vert3->worldPos - vert1->worldPos).xyz()).normalized();
			shaderInput->normal = normal;
	...
	double RemovalFaceZSortApp::CalculateEdge(const Vec2& p, const Vec2& origin, const float dx, const float dy) const
	{
		return (p.x - origin.x) * dy - (p.y - origin.y) * dx;
	}

	bool RemovalFaceZSortApp::IsInside(const Vec2& p, const Vec2& origin, const double dx, const double dy) const
	{
