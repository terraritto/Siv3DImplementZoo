...
{
	void RemovalFaceNoneApp::Prepare()
	{
	...
	void RemovalFaceNoneApp::Render()
	{
	...
	void RemovalFaceNoneApp::FillDrawProcess(const Array<std::shared_ptr<RemovalFaceVertexOutput>>& tempVertices, const Array<unsigned int>& tempIndices)
	{
	...
	double RemovalFaceNoneApp::CalculateEdge(const Vec2& p, const Vec2& origin, const float dx, const float dy) const
	{
		return (p.x - origin.x) * dy - (p.y - origin.y) * dx;
	}

	bool RemovalFaceNoneApp::IsInside(const Vec2& p, const Vec2& origin, const double dx, const double dy) const
	{
