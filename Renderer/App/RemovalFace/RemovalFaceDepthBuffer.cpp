#include "RemovalFaceDepthBuffer.h"
...
{
	void RemovalFaceDepthBufferApp::Prepare()
	{
	...
	void RemovalFaceDepthBufferApp::Render()
	{
	...
	void RemovalFaceDepthBufferApp::FillDrawProcess(const Array<std::shared_ptr<RemovalFaceVertexOutput>>& tempVertices, const Array<unsigned int>& tempIndices)
	{
		...
		for (unsigned int i = 0; i < tempIndices.size(); i += 3)
		{
			...
			for (int x = BBoxX.x; x <= BBoxX.y; x++)
			{
				for (int y = BBoxY.x; y <= BBoxY.y; y++)
				{
					...
					shaderInput->normal = Cross((vert2->worldPos - vert1->worldPos).xyz(), (vert3->worldPos - vert1->worldPos).xyz()).normalized();
	...
	double RemovalFaceDepthBufferApp::CalculateEdge(const Vec2& p, const Vec2& origin, const float dx, const float dy) const
	{
		return (p.x - origin.x) * dy - (p.y - origin.y) * dx;
	}

	bool RemovalFaceDepthBufferApp::IsInside(const Vec2& p, const Vec2& origin, const double dx, const double dy) const
	{
