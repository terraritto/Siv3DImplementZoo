...
namespace TerakoyaRenderer::UV
{
	void SimpleUVApp::Prepare()
	{
		...
		m_vertexShader = std::make_unique<SimpleUVVertexShader>();
		m_pixelShader = std::make_unique<SimpleUVPixelShader>();
		m_pixelShader->SetImage(&m_renderImage);

		if (auto shaderInput = std::static_pointer_cast<SimpleUVVertexInput>(m_vertexShader->GetShaderInput().lock()))
		{
	...
	void SimpleUVApp::Render()
	{
		for (auto& model : m_modelList)
		{
			...
			while (nodes.empty() == false)
			{
				...
				Array<std::shared_ptr<SimpleUVVertexOutput>> tempVertexOutputs;
				...
				for (auto& primitive : primitives)
				{
					...
					{
						auto shaderInput = std::static_pointer_cast<SimpleUVVertexInput>(m_vertexShader->GetShaderInput().lock());
						...
						auto shaderOutput = std::static_pointer_cast<SimpleUVVertexOutput>(m_vertexShader->GetShaderOutput().lock());
						...
						tempVertexOutputs[i] = std::make_shared<SimpleUVVertexOutput>(*shaderOutput);
						i++;
					}
	...
	void SimpleUVApp::FillDrawProcess(const Array<std::shared_ptr<SimpleUVVertexOutput>>& tempVertices, const Array<unsigned int>& tempIndices, std::weak_ptr<DefaultMaterial> material)
	{
		auto pMat = material.lock();

		auto shaderInput = std::static_pointer_cast<SimpleUVPixelInput>(m_pixelShader->GetShaderInput().lock());
	...
	double SimpleUVApp::CalculateEdge(const Vec2& p, const Vec2& origin, const float dx, const float dy) const
	{
		return (p.x - origin.x) * dy - (p.y - origin.y) * dx;
	}

	bool SimpleUVApp::IsInside(const Vec2& p, const Vec2& origin, const double dx, const double dy) const
	{
